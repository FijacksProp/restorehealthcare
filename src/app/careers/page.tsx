...
export const metadata: Metadata = { title: "Careers", description: "Learn about future care and support opportunities with Restore Healthcare Solutions in Dublin 15." };
...
export default function CareersPage() {
  ...
  return (
    <>
      <PageHero eyebrow="Careers at Restore" title="Bring your professionalism. Keep your humanity." intro="We want to build a team that sees the individual, respects the details and understands that excellent support is both skilled and deeply human." />
      <section className="careers-intro"><div className="container"><SectionHeading eyebrow="Work with purpose" title="Help make a good day more possible." text="Future opportunities may include care, support, coordination and operational roles serving individuals and partner organisations across Dublin 15." /><div className="roles-grid">{roles.map(({ icon: Icon, title, text }) => <div className="role-card" key={title}><span><Icon size={22} /></span><h3>{title}</h3><p>{text}</p></div>)}</div></div></section>
      ...
      <CareCta title="Looking for care rather than a career?" text="Explore the support Restore is developing for individuals and families in Dublin 15." />
    </>
  );
}
